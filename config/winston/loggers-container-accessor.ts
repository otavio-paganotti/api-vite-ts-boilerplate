...
import { developmentFormatter, productionFormatter } from '@config/winston/winston-formatter';
import { LoggerOptions } from "@config/winston/get-default-logger";
const {
...
const createLoggerWithOptions = (options: LoggerOptions) => {
  const loggerOptions = {
      transports: [
          new (transports.Console)(),
      ],
      level: options.logLevel,
      format: options.env === 'prd'
